...
def read_elhub_data(base_path=None, meter_dirs=None):
    ...
    for meter_dir in meter_dirs:
        ...
        for csv_file in csv_files:
            try:
                ...
                if 'KWH 60 Forbruk' in df.columns:
                    df['KWH 60 Forbruk'] = df['KWH 60 Forbruk'].str.replace(',', '.').astype(float)
        ...
        if dfs:
            ...
            print(f"Processed {len(dfs)} files for {meter_dir}, final DataFrame shape: {concatenated_df.shape}")
...
def get_consumption_data(meter_name=None):
    ...
    if meter_name is not None:
        return all_data.get(meter_name)
